...
public class DecisionTrainingRecordService {
    ...
    public DecisionTrainingRecordService(
            DecisionTrainingRecordRepository repository
    ) {
    ...
    public DecisionTrainingRecord labelRecord(
        ...
        String observedTrafficLevel =
                normalizeLevel(
                        request.getObservedTrafficLevel(),
                        "Nivelul real de trafic"
                );

        String observedDelayRisk =
                normalizeLevel(
                        request.getObservedDelayRisk(),
                        "Riscul real de intarziere"
                );

        record.setObservedTrafficLevel(
                observedTrafficLevel
        );

        record.setObservedDelayRisk(
                observedDelayRisk
        );

        record.setActualWaiters(
                request.getActualWaiters()
        );

        record.setActualKitchenStaff(
                request.getActualKitchenStaff()
        );

        record.setActualBarStaff(
                request.getActualBarStaff()
        );
    ...
    private String normalizeLevel(
        ...
        if (!ALLOWED_LEVELS.contains(normalizedValue)) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST,
                    fieldName
                            + " trebuie sa fie SCAZUT, MEDIU sau RIDICAT."
            );
        }
