...
public class DecisionService {
    ...
    public DecisionService(OrderService orderService, EmployeeShiftService employeeShiftService, TableSessionRepository tableSessionRepository, DecisionTrainingRecordRepository decisionTrainingRecordRepository, @Value("${ai.service.retrain-token}") String retrainToken) {
    ...
    public DecisionResponse getDecisionSummary() {
        ...
        try {
            ...
            if (response == null) {
                LOGGER.warn("AI Service a returnat un răspuns gol. " + "Se folosește răspunsul de rezervă.");
            ...
            applyStaffingComparison(request, response);

            LOGGER.info("Predicția AI a fost primită cu succes.");
            ...
            LOGGER.warn("AI Service nu este disponibil. " + "Se folosește răspunsul de rezervă: {}", exception.getMessage());
    ...
    public ResponseEntity<String> retrainModels() {
        ...
        LOGGER.info("A fost trimisă cererea de reantrenare.");

        try {
            ResponseEntity<String> response = retrainingRestTemplate.exchange(AI_RETRAIN_URL, HttpMethod.POST, request, String.class);

            LOGGER.info("Reantrenarea modelelor s-a încheiat " + "cu statusul {}.", response.getStatusCode());
            ...
            LOGGER.warn("Cererea de reantrenare a fost respinsă " + "cu statusul {}.", exception.getStatusCode());

            LOGGER.debug("Răspunsul AI Service la reantrenare: {}", exception.getResponseBodyAsString());
            ...
            LOGGER.warn("AI Service nu este disponibil " + "pentru reantrenare: {}", exception.getMessage());
    ...
    private void saveDecisionTrainingRecord(DecisionRequest request, DecisionResponse response) {
        ...
        LOGGER.debug("Înregistrarea pentru reantrenare " + "a fost salvată.");
    }
    ...
    private DecisionRequest buildCurrentDecisionRequest() {
        ...
        ActiveStaffSummaryResponse activeStaffSummary = employeeShiftService.getActiveStaffSummary();

        int activeWaiters = Math.toIntExact(activeStaffSummary.getWaiters());

        int activeKitchen = Math.toIntExact(activeStaffSummary.getKitchenEmployees());

        int activeBar = Math.toIntExact(activeStaffSummary.getBarEmployees());

        LOGGER.debug("Date AI: dayOfWeek={}, hour={}, " + "activeOrders={}, occupiedTables={}, " + "estimatedOccupancy={}, kitchenLoad={}, " + "barLoad={}, avgPreparationTime={}, " + "ordersLast30Min={}, orderAgeMinutes={}, " + "itemCount={}, activeWaiters={}, " + "activeKitchen={}, activeBar={}", dayOfWeek, hour, activeOrders, occupiedTables, estimatedOccupancy, kitchenLoad, barLoad, avgPreparationTime, ordersLast30Min, orderAgeMinutes, itemCount, activeWaiters, activeKitchen, activeBar);

        return new DecisionRequest(dayOfWeek, hour, activeOrders, occupiedTables, estimatedOccupancy, kitchenLoad, barLoad, avgPreparationTime, ordersLast30Min, orderAgeMinutes, itemCount, activeWaiters, activeKitchen, activeBar);
    }
    ...
    private void applyStaffingComparison(DecisionRequest request, DecisionResponse response) {
        int activeWaiters = request.getActiveWaiters();

        int activeKitchen = request.getActiveKitchen();

        int activeBar = request.getActiveBar();

        int waiterDeficit = response.getRecommendedWaiters() - activeWaiters;

        int kitchenDeficit = response.getRecommendedKitchenStaff() - activeKitchen;

        int barDeficit = response.getRecommendedBarStaff() - activeBar;

        /*
         * Un surplus într-un rol nu anulează automat
         * deficitul din alt rol.
         */
        int totalStaffDeficit = Math.max(waiterDeficit, 0) + Math.max(kitchenDeficit, 0) + Math.max(barDeficit, 0);

        response.setActiveWaiters(activeWaiters);

        response.setActiveKitchenStaff(activeKitchen);

        response.setActiveBarStaff(activeBar);

        response.setWaiterDeficit(waiterDeficit);

        response.setKitchenDeficit(kitchenDeficit);

        response.setBarDeficit(barDeficit);

        response.setTotalStaffDeficit(totalStaffDeficit);
    }

    private DecisionResponse buildFallbackResponse(DecisionRequest request) {
        ...
        response.setActiveWaiters(request.getActiveWaiters());

        response.setActiveKitchenStaff(request.getActiveKitchen());

        response.setActiveBarStaff(request.getActiveBar());
