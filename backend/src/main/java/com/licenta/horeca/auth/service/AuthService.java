...
public class AuthService {
    ...
    public AuthService(UserRepository userRepository, PasswordEncoder passwordEncoder, JwtService jwtService, EmployeeShiftService employeeShiftService) {
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        User user = userRepository.findByEmail(request.getEmail()).orElseThrow(() -> new BusinessException(INVALID_CREDENTIALS_MESSAGE));
         ...
         */
        employeeShiftService.ensureShiftForLogin(user.getId());

        String authority = "ROLE_" + user.getRole().getName().name();

        UserDetails userDetails = org.springframework.security.core.userdetails.User.withUsername(user.getEmail()).password(user.getPassword()).authorities(authority).disabled(!user.isActive()).build();

        String token = jwtService.generateToken(userDetails);

        return new LoginResponse(user.getId(), user.getFullName(), user.getEmail(), user.getRole().getName().name(), token);
    }
}