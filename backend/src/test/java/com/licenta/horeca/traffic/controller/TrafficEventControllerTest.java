...
class TrafficEventControllerTest {
    ...
    void registerEntryShouldReturnEntryEvent() throws Exception {
        ...
        when(trafficEventService.saveEvent(TrafficEventType.ENTRY)).thenReturn(event);

        mockMvc.perform(post(ENTRY_ENDPOINT)).andExpect(status().isOk()).andExpect(jsonPath("$.type").value("ENTRY"));
    }
    ...
    void registerExitShouldReturnExitEvent() throws Exception {
        ...
        when(trafficEventService.saveEvent(TrafficEventType.EXIT)).thenReturn(event);

        mockMvc.perform(post(EXIT_ENDPOINT)).andExpect(status().isOk()).andExpect(jsonPath("$.type").value("EXIT"));
    }
    ...
    void getAllEventsShouldReturnEvents() throws Exception {
        ...
        when(trafficEventService.getAllEvents()).thenReturn(List.of(entryEvent, exitEvent));

        mockMvc.perform(get(TRAFFIC_ENDPOINT)).andExpect(status().isOk()).andExpect(jsonPath("$.length()").value(2)).andExpect(jsonPath("$[0].type").value("ENTRY")).andExpect(jsonPath("$[1].type").value("EXIT"));
    }
    ...
    void getSummaryShouldReturnTrafficSummary() throws Exception {
        ...
        mockMvc.perform(get(SUMMARY_ENDPOINT)).andExpect(status().isOk()).andExpect(jsonPath("$.entries").value(10)).andExpect(jsonPath("$.exits").value(4)).andExpect(jsonPath("$.estimatedOccupancy").value(6));
    }
}
