...
function ManagerPage() {
  ...
  const newOrdersCount = activeOrders.filter((order) => order.status === "NOUA").length;

  const inPreparationOrdersCount = activeOrders.filter((order) => order.status === "IN_PREPARARE").length;

  const readyOrdersCount = activeOrders.filter((order) => order.status === "GATA").length;
          ...
          <div className="manager-grid">
            {productSalesList.slice(0, 3).map((product) => (
              <div key={product.name} className="manager-card">
                ...
                <p>
                  Produsul a fost vandut de <strong>{product.quantity}</strong> ori.
                </p>
              </div>
            ))}
          </div>
