...
function AdminPage() {
  ...
  const [unavailableSupplies, setUnavailableSupplies] =
    useState([]);
  ...
  const [labelMessage, setLabelMessage] =
    useState("");
  ...
  const [actualWaiters, setActualWaiters] =
    useState("0");
  ...
  const [actualBarStaff, setActualBarStaff] =
    useState("0");

  const [retraining, setRetraining] =
    useState(false);
  ...
  const loadLatestUnlabeledRecord = () => {
    ...
    getLatestUnlabeledDecisionRecord()
      ...
      .catch((error) => {
        ...
        console.error(
          "Eroare la incarcarea predictiei neetichetate:",
          error
        );

        setLabelError(
          "Ultima predictie neetichetata nu a putut fi incarcata."
        );
      })
      .finally(() => {
  ...
  const loadDecisionSummary = () => {
    ...
    getDecisionSummary()
      ...
      .catch((error) => {
        console.error(
          "Eroare la incarcarea predictiilor:",
          error
        );
  ...
  const handleLabelSubmit = (event) => {
    ...
    if (!latestUnlabeledRecord?.id) {
      setLabelError(
        "Nu exista nicio predictie care poate fi etichetata."
      );
      return;
    }
    ...
    if (
      ...
      setLabelError(
        "Completeaza numarul real de angajati."
      );
      return;
    }
    ...
    if (
      ...
      setLabelError(
        "Numarul angajatilor trebuie sa fie zero sau mai mare."
      );
      return;
    }
    ...
    labelDecisionRecord(
      latestUnlabeledRecord.id,
      labelData
    )
      .then(() => {
        setLabelMessage(
          `Inregistrarea #${latestUnlabeledRecord.id} a fost etichetata cu succes.`
        );
      ...
      .catch((error) => {
        console.error(
          "Eroare la salvarea label-urilor:",
          error
        );

        if (error.response?.status === 409) {
          setLabelError(
            "Aceasta inregistrare a fost deja etichetata."
          );
        ...
        setLabelError(
          "Rezultatele reale nu au putut fi salvate."
        );
      })
      .finally(() => {
      ...
      .then((response) => {
        setRetrainingMessage(
          response.data?.message ||
            "Reantrenarea a fost finalizata."
        );
      })
      .catch((error) => {
        const message =
  ...
  const filteredOrders = servedOrders.filter(
  ...
  const productSalesList = Object.entries(
    productSales
  )
    .map(([name, quantity]) => ({
      name,
  ...
  return (
    <div className="admin-page">
        ...
        {decisionSummary && (
          <div className="admin-grid">
            ...
            <div className="admin-card">
              ...
              <p>
                {
                  decisionSummary.recommendedWaiters
                }
              </p>
            </div>
      ...
      <section className="admin-section">
        ...
        <p>
          Completeaza rezultatele observate pentru
          ultima predictie. Aceste valori vor fi
          folosite ca label-uri la reantrenarea
          modelelor.
        </p>

        {labelLoading && (
          <p>
            Se cauta ultima predictie
            neetichetata...
          </p>
        )}
        ...
        {!labelLoading &&
          !latestUnlabeledRecord && (
            <p>
              Nu exista momentan nicio predictie
              neetichetata. Apasa „Actualizeaza
            ...
            >
              <div className="decision-label-grid">
                <div className="filter-group">
                  <label htmlFor="observed-traffic">
                    Trafic observat
                  </label>
                ...
                <div className="filter-group">
                  <label htmlFor="actual-kitchen-staff">
                    Personal bucatarie prezent
                  </label>
                ...
                <div className="filter-group">
                  <label htmlFor="actual-bar-staff">
                    Personal bar prezent
                  </label>
        ...
        <div className="retraining-area">
          ...
          <p>
            Reantrenarea este permisa dupa
            colectarea a cel putin 30 de
      ...
      <section className="admin-section">
        ...
        {unavailableSupplies.length === 0 ? (
          <p>
            Nu exista produse auxiliare semnalate
            ca lipsa.
          </p>
        ) : (
          <div className="admin-grid">
