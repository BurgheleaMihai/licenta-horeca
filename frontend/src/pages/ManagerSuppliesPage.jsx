...
function ManagerSuppliesPage() {
  ...
  const [entryForm, setEntryForm] =
    useState(initialEntryForm);

  const [editingEntryId, setEditingEntryId] =
    useState(null);

  const [entryHistory, setEntryHistory] =
    useState([]);
  ...
  const targetSupply = useMemo(() => {
    ...
    return (
      supplies.find(
        (supply) =>
          supply.id === Number(entryForm.supplyId)
      ) || selectedSupply
    );
  }, [
    entryForm.supplyId,
  ...
  const loadSupplies = () => {
    ...
    getAllActiveAuxiliarySupplies()
      ...
      .catch((error) => {
        console.error(
          "Eroare la incarcarea stocurilor:",
          error
        );

        setErrorMessage(
          "Stocurile nu au putut fi incarcate."
        );
      });
  };
  ...
  const loadEntryHistory = (supplyId) => {
    ...
    getStockEntries(supplyId)
      ...
      .catch((error) => {
        ...
        setErrorMessage(
          "Istoricul intrarilor nu a putut fi incarcat."
        );
      })
      .finally(() => {
  ...
  const getSelectedVariantForGroup = (
    group
  ) => {
    const selectedId = Number(
      selectedVariantIds[group.key]
    );
  ...
  const handlePackageTypeChange = (event) => {
    ...
    setEntryForm((currentForm) => ({
      ...
      quantityPerPackage:
        newPackageType === "DIRECT"
          ? "1"
          : currentForm.quantityPerPackage,
    }));
  };

  const validateEntryForm = () => {
    const packageQuantity = Number(
      entryForm.packageQuantity
    );

    const quantityPerPackage = Number(
      entryForm.quantityPerPackage
    );

    if (
      Number.isNaN(packageQuantity) ||
      packageQuantity <= 0
    ) {
      setErrorMessage(
    ...
    if (
      entryForm.packageType !== "DIRECT" &&
      (
        Number.isNaN(quantityPerPackage) ||
    ...
    if (
      ...
      setErrorMessage(
        "Varianta destinatie este obligatorie."
      );
  ...
  const handleEditEntry = (entry) => {
    ...
    setEntryForm({
      ...
      quantityPerPackage: String(
        entry.quantityPerPackage ?? 1
      ),

      inputUnit:
        entry.inputUnit ||
        selectedSupply.baseUnit,
  ...
  const handleDeleteEntry = (entry) => {
    ...
    deleteStockEntry(entry.id)
      ...
      .catch((error) => {
        console.error(
          "Eroare la stergerea intrarii:",
          error
        );

        setErrorMessage(
          error.response?.data?.message ||
            "Intrarea nu a putut fi stearsa."
        );
      })
      .finally(() => {
  ...
  const handleMarkUnavailable = (
    ...
    markSupplyUnavailable(supplyId)
      .then(() => {
        setSuccessMessage(
          "Lipsa a fost semnalata pentru varianta selectata."
        );
      ...
      .catch((error) => {
        console.error(
          "Eroare la semnalarea lipsei:",
          error
        );

        setErrorMessage(
          "Lipsa nu a putut fi semnalata."
        );
      });
  };
  ...
  const handleMarkAvailable = (supplyId) => {
    ...
    markSupplyAvailable(supplyId)
      .then(() => {
        setSuccessMessage(
          "Varianta a fost marcata disponibila."
        );
      ...
      .catch((error) => {
        console.error(
          "Eroare la actualizarea variantei:",
          error
        );

        setErrorMessage(
          "Varianta nu a putut fi marcata disponibila."
        );
      });
  };
  ...
  return (
    <div className="manager-page">
        ...
        <section className="manager-section">
          ...
          <div className="stock-entry-history">
            ...
            {loadingHistory ? (
              ...
              <p>
                Nu exista intrari salvate pentru
                aceasta varianta.
              </p>
            ) : (
      ...
      {stockTypes.map((stockType) => {
        const groupsFromZone =
          groupedSupplies.filter(
            (group) =>
              group.stockType === stockType
          );
        ...
        return (
          ...
          >
            ...
            {groupsFromZone.length === 0 ? (
              <p>
                Nu exista produse configurate in
                aceasta zona.
              </p>
            ) : (
              <div className="manager-grid">
